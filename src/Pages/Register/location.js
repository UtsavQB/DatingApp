    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Pass the coordinates as [longitude, latitude]
          fetchData([longitude, latitude]);
        },
        (error) => {
          console.error("Geolocation error:", error);
          alert(
            "Location access denied. Please allow location access to see nearby places."
          );
        }
      );
    } else {
...
const getdi = localStorage.getItem('id')
console.log(getdi,'id')
const fetchData = async (coordinates) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/users/${getdi}/location/live`,
        {
          method: "POST",
      ...
      <div className="bg-gradient-to-br from-purple-400 to-purple-500 shadow-lg rounded-lg w-full max-w-md h-[400px] p-8 flex flex-col items-center justify-between mx-auto space-y-4">
        ...
        <div className="flex flex-col items-center space-y-10 mb-0">
          ...
          <h1 className="text-center text-md font-medium text-black">
            {locationEnabled ? (
              <>
                You have enabled your location! We can now show you a list of
                places nearby.
                <br />
